...
# Helper: pin physical NIC names by MAC address inside the initramfs.
#
# WHY: the initramfs occasionally fails to apply predictable interface names.
# For example the ixgbe SLOT policy (ID_NET_NAME_SLOT) intermittently does not
# yield a name because the ACPI/PCI hotplug slot data isn't ready in time, so
# the NIC keeps its kernel default name (eth0/eth1) for that boot. When that
# happens, NM connection profiles bound to interface-name=<X> (and any VLAN /
# bond / bridge whose parent chains back to <X>) never match, networking never
# comes up, and the cloudflared tunnel can't start — the remote unlock page is
# unreachable. Matching a .link by MAC address does NOT depend on the flaky
# slot/path data, so the name becomes deterministic on every boot.
#
# We generate a .link for every *physical* device referenced by a copied NM
# profile's interface-name=. Virtual devices (vlan/bond/bridge/team) are skipped:
# they are created by NM by name and have no backing hardware/MAC to match.
_pin_copied_iface_names() {
    local conn_dir="${initdir}/etc/NetworkManager/system-connections"
    local netdir="${initdir}/etc/systemd/network"
    local prof ifn mac linkfile
    [ -d "$conn_dir" ] || return 0
    for prof in "$conn_dir"/*.nmconnection; do
        [ -e "$prof" ] || continue
        ifn=$(sed -n 's/^interface-name=//p' "$prof" 2>/dev/null | sed -n '1p')
        [ -n "$ifn" ] || continue
        # Only pin real physical devices (they have a backing device + MAC).
        [ -e "/sys/class/net/${ifn}/device" ] || continue
        mac=$(cat "/sys/class/net/${ifn}/address" 2>/dev/null)
        [ -n "$mac" ] || continue
        mkdir -p "$netdir"
        linkfile="${netdir}/71-dracut-cloudflared-ttyd-${ifn}.link"
        cat > "$linkfile" <<LINKEOF
# Generated by dracut-cloudflared-ttyd: pin ${ifn} by MAC so the initramfs
# names this NIC identically to the host every boot, even when the predictable-
# naming policy (slot/path) intermittently fails to yield a name (which would
# leave the kernel default eth0/eth1 and break interface-name= bound NM profiles
# and their VLAN/bond/bridge children).
[Match]
MACAddress=${mac}
...
Name=${ifn}
LINKEOF
        dinfo "dracut-cloudflared-ttyd: Pinned NIC '${ifn}' -> MAC '${mac}' (${linkfile##*/})"
    done
}
...
NMWIFI
    ...
    # ---- Pin physical NIC names by MAC for deterministic naming at boot ----
    # Ensures the initramfs always names the uplink NIC(s) the same as the host,
    # so the copied NM profiles (interface-name=...) and their VLAN/bond children
    # match even when predictable naming (slot/path) flakes and would otherwise
    # leave the kernel default eth0/eth1.
    _pin_copied_iface_names
